...
import 'package:vinayaga_project/master/windingView2.dart';
...
class WithPrinting extends StatefulWidget {
  const WithPrinting({super.key});

  @override
  State<WithPrinting> createState() => _WithPrintingState();
}

//printwith

class _WithPrintingState extends State<WithPrinting> {
  @override
...
*/
  ...
  Future<void> fetchWindingreport() async {
    try {
      final url = Uri.parse('http://localhost:3309/winding_entry_get_report_With_Print/');
      final response = await http.get(url);
                                      ...
                                      TextButton(
                                        ...
                                        onPressed: () {
                                          Navigator.push(context,
                                              MaterialPageRoute(builder: (context) =>const WithPrinting()));// Close the alert box
                                        },
                                      ),
                                      TextButton(
                                        child: const Text('No'),
        ...
        Row(
          children: [
            IconButton(icon: Icon(Icons.edit,color:Colors. blue,),onPressed: (){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>WindingView2(
                id:row["id"],
...
class _FinishingEntryState extends State<FinishingEntry> {
  ...
  Future<void> filterFromtodateData4(
    ...
    try {
      final url = Uri.parse(
          'http://localhost:3309/get_fromtodate4?fromDate=$fromDate&toDate=$toDate&shiftType=$shiftType');
      final response = await http.get(url);
                            ...
                            onPressed: (){
                              if(_formKey.currentState!.validate()){
                                ...
                                if(dropdownvalue == "Shift Type"){
                                  ...
                                  setState(() {
                                    errorMessage = '* Enter a correct Person 2';
                                  });
                                }
