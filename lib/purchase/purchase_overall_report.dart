...
class PurchaseOverallReport extends StatefulWidget {
  int sn = 0;
  ...
  PurchaseOverallReport({super.key,
  ...
  @override
  State<PurchaseOverallReport> createState() => _PurchaseOverallReportState();
}


class _PurchaseOverallReportState extends State<PurchaseOverallReport> {

  double calculateTotal(List<Map<String, dynamic>> filteredData) {
    double total = 0;
    for (var row in filteredData) {
      double grandTotal = double.parse(row['grandTotal'] ?? '0');
      double returnTotal = double.parse(row['returnTotal'] ?? '0');
      total += grandTotal - returnTotal;
    }
    return total;
  }
  double getTotal() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Purchase Report PDF"), centerTitle: true,),
      body: PdfPreview(
