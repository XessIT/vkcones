...
import 'package:vinayaga_project/report/po_pending_view.dart';
...
class POPendingReport extends StatefulWidget {
  const POPendingReport({Key? key}) : super(key: key);
  @override
  State<POPendingReport> createState() => _POPendingReportState();
}
class _POPendingReportState extends State<POPendingReport> {
  ...
  Future<void> fetchData() async {
    try {
      final url = Uri.parse('http://localhost:3309/get_po_pending_report/');
  ...
  void initState() {
    ...
    filteredData = List<Map<String, dynamic>>.from(data);
    // Initialize fromDate and toDate with some default values
  }
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () {
                                      Navigator.push(context, MaterialPageRoute(builder: (context)=>POPendingReport()));
                                    },
                                  ),
                                  IconButton(
        ...
        Row(
          children: [
            IconButton(icon: Icon(Icons.remove_red_eye_outlined,color:Colors. blue,),onPressed: (){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>POPendingView(
                pendingOrderNo: row["pendingOrderNo"],
                 date:row["date"].toString(),
                customerName:row["supName"].toString(),
                customerMobile:row["supMobile"].toString(),
                customerAddress:row["supAddress"].toString(),
                customercode:row["supCode"].toString(),
                itemGroup:row["prodCode"].toString(),
                itemName:row["prodName"].toString(),
